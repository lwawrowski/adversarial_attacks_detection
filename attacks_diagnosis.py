...
# wilt - problem z reduktami
# nomao - tylko jeden atak, do sprawdzenia

# list_of_files = ['Bioresponse', 'churn', 'madelon', 'phoneme', 'qsar-biodeg', 'wdbc', 'wilt']
list_of_files = ['cmc']

attacks = ["fgm", "pgd", "bim", "noise"]

# input_file = "qsar-biodeg"
# selected_attack = attacks[0]
# path_diag = os.path.join(directory, input_file, "diagnoses")

# print(os.path.join(directory, input_file, "datasets", selected_attack + ".csv"))
...
for input_file in tqdm(list_of_files, desc="Dataset"):
    ...
    if isinstance(diagnosed_file_df, pd.DataFrame): # and not os.path.exists(os.path.join(path_diag, "org.csv")):
